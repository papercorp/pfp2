...
<main>
  ...
  <ul>
    <li><strong>What is the problem being solved?:</strong> Opensea has delisted the original <a href="https://pfp.factoria.app">PFP collection</a> for copyright violation. It looks like one of the minters minted an image of an already existing NFT. Instead of just delisting that specific NFT, Opensea has delisted the entire collection wholesale. I have reached out to them to appeal but never received a response. Because Twitter Blue uses Opensea's proprietary API instead of using the blockchain, this means the PFP NFT holders won't be able to use the hexagon avatar anymore.</li>
